  ...
  create() {
    ...
    this.socket.on('autoRestart', function(velocity) {
      if (ball) {
        ...
        //values will need to be changed if score bounds change!!
        if ( velocity.bottom /*&& ((ball.y <= 30) || (ball.y >= 550))*/) {
          bottomScore+= 0.25;
          bottomScoreText.setText('Score: ' + Math.floor(bottomScore));
        } else /*if (!velocity.bottom && ((ball.y <= 30) || (ball.y >= 550)))*/{
          topScore+= 0.25;
...
//all below is new
  ...
  update(time,delta) {
    ...
    if (ball) {
      ...
      if (ball.body.y < 30) {
        //console.log('here about to restart');
        // if (!caught) {
        //   caught = true;
        // }
        this.socket.emit('ballRestart',{x:0,y:250, bottom: true});
      } else if (ball.body.y >= 550) {
        // if(!caught) {
        //   caught=true;
        // }
        //console.log('again about to restart');
